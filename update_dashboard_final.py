...
def main():
    ...
    if target_date:
        ...
        if not collateral_files:
            print(f"ERROR: No Collateral calculator CSV found")
            return 1
    ...
    if os.path.exists(shared_dir):
        for file_name in ["collateral_dashboard.html", "collateral_dashboard.pptx", "index.html"]:
            src = os.path.join(script_dir, file_name)
            dst = os.path.join(shared_dir, file_name)
            if os.path.exists(src):
                try:
                    shutil.copy2(src, dst)
                    print(f"[OK] Copied {file_name} to Shared Drive")
                except Exception as e:
                    print(f"Warning: Failed to copy {file_name} to Shared Drive: {e}")
    else:
