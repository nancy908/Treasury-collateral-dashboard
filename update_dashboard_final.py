...
def parse_collateral_csv(csv_path):
    ...
    if not as_of_date:
        ...
        dated_files = glob.glob(os.path.join(csv_dir, 'Collateral calculator 2 - Summary *.csv'))
        for df in sorted(dated_files, key=os.path.getctime, reverse=True):
            m = re.search(r'(\d{4}-\d{2}-\d{2})', os.path.basename(df))
            if m:
                as_of_date = m.group(1)
                break
...
def main():
    ...
    if target_date:
        ...
        search_dirs = [csv_dir, os.path.join(csv_dir, "Older")]
        
        matched_files = []
        for d in search_dirs:
            if os.path.exists(d):
                matched_files.extend(glob.glob(os.path.join(d, f'*{target_date}*.csv')))
        
        collateral_path = next((f for f in matched_files if 'Collateral calculator' in os.path.basename(f)), None)
        treasury_path = next((f for f in matched_files if 'CFF Treasury' in os.path.basename(f)), None)
        fpa_path = next((f for f in matched_files if 'CFF FP&A' in os.path.basename(f)), None)

        if not collateral_path:
            print(f"ERROR: No Collateral calculator CSV found for date {target_date}")
            return 1
    else:
        # Check for fixed filenames first, fallback to latest dated files
        fixed_collateral = os.path.join(csv_dir, 'Collateral calculator 2 - Summary.csv')
        fixed_treasury = os.path.join(csv_dir, 'CFF Treasury.csv')
        fixed_fpa = os.path.join(csv_dir, 'CFF FP&A.csv')

        collateral_files = glob.glob(os.path.join(csv_dir, 'Collateral calculator 2 - Summary *.csv'))
        treasury_files = glob.glob(os.path.join(csv_dir, 'CFF Treasury *.csv'))
        fpa_files = glob.glob(os.path.join(csv_dir, 'CFF FP&A *.csv'))

        if os.path.exists(fixed_collateral):
            collateral_path = fixed_collateral
            treasury_path = fixed_treasury if os.path.exists(fixed_treasury) else (max(treasury_files, key=os.path.getctime) if treasury_files else None)
            fpa_path = fixed_fpa if os.path.exists(fixed_fpa) else (max(fpa_files, key=os.path.getctime) if fpa_files else None)
        elif collateral_files:
            collateral_path = max(collateral_files, key=os.path.getctime)
            treasury_path = max(treasury_files, key=os.path.getctime) if treasury_files else None
            fpa_path = max(fpa_files, key=os.path.getctime) if fpa_files else None
        else:
            print(f"ERROR: No Collateral calculator CSV found at {fixed_collateral}")
            return 1
